...
# §11.4.85 chaos: malformed-request salvo + oversized header +
# slow-loris connections injected concurrently with legit load; the
# HelixAgent platform MUST refuse cleanly / degrade gracefully and
# never crash. No-target / unreachable → honest SKIP-OK (§11.4.3).
# Every PASS cites captured evidence.

set -uo pipefail
HEALTH_URL="${HELIXAGENT_HEALTH_URL:-}"
CHAOS_HOST="${HELIXAGENT_CHAOS_HOST:-localhost}"
CHAOS_PORT="${HELIXAGENT_CHAOS_PORT:-}"
LEGIT_REQS="${CHAOS_LEGIT_REQUESTS:-100}"
MIN_PCT="${CHAOS_LEGIT_MIN_PASS_PCT:-95}"

echo "=== HelixAgent Chaos Failure-Injection Challenge ==="
echo "  url=$HEALTH_URL host=${CHAOS_HOST}:${CHAOS_PORT}"

if [[ -z "$HEALTH_URL" ]] || [[ -z "$CHAOS_PORT" ]]; then
    echo "[1/6] SKIP: HELIXAGENT_HEALTH_URL/_CHAOS_PORT unset — SKIP-OK: #env-no-target"
    echo "=== HelixAgent Chaos Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== HelixAgent Chaos Challenge: PASSED ==="
