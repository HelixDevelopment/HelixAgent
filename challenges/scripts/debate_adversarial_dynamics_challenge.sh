...
if [ -f "$PROJECT_ROOT/DebateOrchestrator/agents/adversarial.go" ]; then
...
if (cd "$PROJECT_ROOT/DebateOrchestrator" && go build ./agents/... 2>&1); then
...
if grep -q "type AttackReport struct" "$PROJECT_ROOT/DebateOrchestrator/agents/adversarial.go" 2>/dev/null; then
...
if grep -q "type DefenseReport struct" "$PROJECT_ROOT/DebateOrchestrator/agents/adversarial.go" 2>/dev/null; then
...
if grep -q "type AdversarialProtocol struct" "$PROJECT_ROOT/DebateOrchestrator/agents/adversarial.go" 2>/dev/null; then
...
if grep -q "type Vulnerability struct" "$PROJECT_ROOT/DebateOrchestrator/agents/adversarial.go" 2>/dev/null; then
...
if grep -q "func NewAdversarialProtocol" "$PROJECT_ROOT/DebateOrchestrator/agents/adversarial.go" 2>/dev/null; then
...
if grep -q "func (ap \*AdversarialProtocol) Execute" "$PROJECT_ROOT/DebateOrchestrator/agents/adversarial.go" 2>/dev/null; then
...
if [ -f "$PROJECT_ROOT/DebateOrchestrator/agents/adversarial_test.go" ]; then
...
if (cd "$PROJECT_ROOT/DebateOrchestrator" && nice -n 19 ionice -c 3 go test -short -count=1 -p 1 -timeout 120s ./agents/ -run "TestAdversarial|TestAttack|TestDefense|TestRedBlue" 2>&1 | tail -5 | grep -q "^ok\|PASS"); then
