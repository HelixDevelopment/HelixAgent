...
# HelixAgent per CONST-035 + CONST-050(B). Submodule cascade per
# CONST-051(A). Anti-bluff covenant §11.4 / §11.4.69 / §11.4.85.
# Targets the HelixAgent platform health endpoint at
# $HELIXAGENT_HEALTH_URL and proves it survives a concurrent request
# flood without dropping below the pass threshold or dying.
#
...
echo "=== HelixAgent DDoS Health-Flood Challenge ==="
echo "  url=$HEALTH_URL total=$TOTAL_REQS conc=$CONCURRENCY pass≥${MIN_PASS_PCT}%"

if [[ -z "$HEALTH_URL" ]]; then
    echo "[1/5] SKIP: HELIXAGENT_HEALTH_URL unset — SKIP-OK: #env-no-target"
    echo "=== HelixAgent DDoS Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== HelixAgent DDoS Challenge: PASSED ==="
