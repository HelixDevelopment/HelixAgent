...
func TestHelixMemoryLazyServiceChallenge(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping memory lazy-service challenge in short mode (requires live server and lazy services)")
	}
	...
	t.Run("BigDataSubsystemHealth", func(t *testing.T) {
		...
		if resp.StatusCode == http.StatusNotFound {
			t.Skip("BigData subsystem not mounted on this server")
		}
	...
	t.Run("MemorySyncStatus", func(t *testing.T) {
		...
		if resp.StatusCode == http.StatusNotFound {
			t.Skip("Memory sync endpoint not mounted on this server")
		}
	...
	t.Run("KnowledgeGraphSearch", func(t *testing.T) {
		...
		if resp.StatusCode == http.StatusNotFound {
			t.Skip("Knowledge graph endpoint not mounted on this server")
		}
	...
	t.Run("ContextReplayWithMemory", func(t *testing.T) {
		...
		if resp.StatusCode == http.StatusNotFound {
			t.Skip("Context replay endpoint not mounted on this server")
		}
	...
	t.Run("LearningInsightsAndPatterns", func(t *testing.T) {
		...
		if resp.StatusCode == http.StatusNotFound {
			t.Skip("Learning subsystem not mounted on this server")
		}
	...
	t.Run("CogneeLazyBoot", func(t *testing.T) {
		...
		if err != nil {
			...
			if !booted {
				t.Skip("Cognee did not lazy-boot within 60 seconds (may require docker-compose.memory.yml)")
			}
		} else {
			resp.Body.Close()
	...
	t.Run("CogneeDataPipeline", func(t *testing.T) {
		...
		if err != nil || resp.StatusCode != http.StatusOK {
			...
			t.Skip("Cognee not available for data pipeline test")
		}
		resp.Body.Close()
	...
	t.Run("ProviderAnalytics", func(t *testing.T) {
		...
		if resp.StatusCode == http.StatusNotFound {
			t.Skip("Analytics endpoint not mounted on this server")
		}
	...
	t.Run("DebateAnalytics", func(t *testing.T) {
		...
		if resp.StatusCode == http.StatusNotFound {
			t.Skip("Deate analytics endpoint not mounted on this server")
		}
