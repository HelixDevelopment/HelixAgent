// Package deepseek provides configuration builders for DeepSeek.
package deepseek
...
// ConfigBuilder implements the ConfigBuilder interface for DeepSeek.
type ConfigBuilder struct{}

// NewConfigBuilder creates a new DeepSeek config builder.
func NewConfigBuilder() *ConfigBuilder {
	return &ConfigBuilder{}
}

// Build builds a DeepSeek configuration from a map.
func (b *ConfigBuilder) Build(config map[string]interface{}) (interface{}, error) {
	deepseekConfig := &Config{
		APIKey:    getString(config, "api_key", ""),
		BaseURL:   getString(config, "base_url", "https://api.deepseek.com"),
...
// Validate validates a DeepSeek configuration.
func (b *ConfigBuilder) Validate(config interface{}) error {
	dsConfig, ok := config.(*Config)
	if !ok {
...
// Merge merges two DeepSeek configurations.
...
// Config represents DeepSeek-specific configuration.
type Config struct {
