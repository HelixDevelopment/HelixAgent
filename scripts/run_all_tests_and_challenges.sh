...
if [ -d "../HelixQA" ] && [ -f "../helix_qa/banks/all-formats.yaml" ]; then
    ...
    ../helix_qa/bin/helixqa --banks ../helix_qa/banks/all-formats.yaml --output-dir "${PROJECT_ROOT}/reports/helixqa" --report-format markdown 2>&1 | tee "${PROJECT_ROOT}/reports/helixqa.log"
