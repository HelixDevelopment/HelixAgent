#!/usr/bin/env bash
# stress_sustained_load_challenge.sh — anti-bluff Stress Challenge for
# HelixAgent per CONST-035 + CONST-050(B). Cascade per CONST-051(A).
# §11.4.85 sustained-load: N≥(DURATION×RPS) requests, p50 baseline vs
# post-load latency-degradation budget. No-target / unreachable →
# honest SKIP-OK (§11.4.3). Every PASS cites captured evidence.

set -uo pipefail
HEALTH_URL="${HELIXAGENT_HEALTH_URL:-}"
...
echo "=== HelixAgent Stress Sustained-Load Challenge ==="
echo "  url=$HEALTH_URL dur=${DURATION}s rps=${RPS}"

if [[ -z "$HEALTH_URL" ]]; then
    echo "[1/6] SKIP: HELIXAGENT_HEALTH_URL unset — SKIP-OK: #env-no-target"
    echo "=== HelixAgent Stress Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo "[1/6] Pre-stress liveness: PASS"
...
echo "[2/6] Baseline median latency: ${base_med}s"

body=$(curl -sS --max-time "$TIMEOUT_SEC" "$HEALTH_URL" 2>/dev/null || true)
printf '%s' "$body" | grep -qE '"status"\s*:\s*"(ok|healthy|UP)"' || { echo "[3/6] FAIL: health schema not recognised"; exit 1; }
...
[[ "$pct" -lt "$MIN_PASS_PCT" ]] && { echo "  FAIL: pass-pct ${pct}% < ${MIN_PASS_PCT}%"; exit 1; }
...
[[ "$deg" -gt "$MAX_DEG_PCT" ]] && { echo "  FAIL: latency degraded ${deg}% > ${MAX_DEG_PCT}%"; exit 1; }

post=$(curl -sS --max-time "$TIMEOUT_SEC" -o /dev/null -w "%{http_code}" "$HEALTH_URL" 2>/dev/null) || post="000"
[[ "$post" != "200" ]] && { echo "[6/6] FAIL: service dead after stress (HTTP $post)"; exit 1; }
echo "[6/6] Post-stress liveness: PASS"

echo
echo "=== HelixAgent Stress Challenge: PASSED ==="
