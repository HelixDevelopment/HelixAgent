...
24. **Canonical Compose Resource Form** — Every service in `docker-compose.yml` MUST declare resources via `deploy.resources.limits` + `deploy.resources.reservations` only. The legacy top-level keys `mem_limit`, `memswap_limit`, and `pids_limit` are forbidden — Docker Compose v2 rejects projects when both forms are set with different values, breaking remote distribution to docker-runtime hosts. Every value MUST use Compose env-var interpolation (`${SERVICE_FIELD:-default}`) so resources scale per environment without YAML edits. Production overrides live in `docker-compose.scale.yml` + `.env.scale.example`. Tier table and rationale: `docs/development/container-resource-policy.md`. Enforced by `challenges/scripts/compose_resource_limits_challenge.sh` (CONST-032 reproduction guard) and `internal/adapters/containers/compose_resources_test.go` (`TestComposeResourceInvariants` under `go test ./...`). Adding a service: register it in `scripts/normalize-compose-resources.py` `TIERS` table, run the script, update the policy doc, and add the name to the test + Challenge required sets. Worked example: `docs/issues/fixed/BUGFIXES.md` Issue #50 (cognee `mem_limit=2g` vs `deploy.resources.limits.memory=4G` blocked amber.local boot).

27. **Anti-Bluff Tests & Challenges (CONST-035)** — **Tests and Challenges MUST verify the product, not the LLM's mental model of the product.** A test that passes when the feature is broken is worse than a missing test — it gives false confidence and lets defects ship. Every test and Challenge MUST be **functional** (exercises the real code path the user will hit) AND **strict** (fails when the feature doesn't actually work). Specifically:

    1. **No soft passes that hide failures.** A reachability check that "trusts the gateway is reachable so we don't probe the backend" passes when the backend is broken. If a service is supposed to listen on a port, the test MUST connect to that port and verify a real protocol response — TCP-open is the FLOOR, not the ceiling. Postgres → `SELECT 1`. Redis → `PING` returning `PONG`. ChromaDB → `GET /api/v1/heartbeat` returning 200. MCP server → TCP connection + valid JSON-RPC handshake. Gateway → `POST /v1/chat/completions` returning 200 with a non-empty completion. If a test cannot exercise the real behavior, it MUST be marked `t.Skip("…SKIP-OK: #<reason>")` (per CLAUDE.md DoD rule 4) — never silently pass.

    2. **No mocks or fakes outside unit tests** (already CONST-030, reaffirmed here). Integration / E2E / Challenge tests MUST hit real running instances. Mocking the database in an integration test is the single biggest source of "tests pass, product broken" — that's why CONST-030 forbids it; CONST-035 escalates a failed-but-passing-test to the same severity as a regression.

    3. **Functional verification of every claim.** When the code claims "service X is reachable on host Y at port Z", the test MUST actually `nc -z Y Z` succeed AND the next protocol-layer probe (`PING`/`SELECT 1`/HTTP request) MUST return a valid response. Just because the container is `Up` doesn't mean the application inside it is serving traffic.

    4. **Re-verify after every change.** Don't assume a previously-passing test still verifies the same scope after a refactor. When you edit code, re-read the tests to confirm they still cover the user-visible behavior — not just the structural shape that happened to pass.

    5. **Apply to all submodules.** Each submodule's `CONSTITUTION.md` / `CLAUDE.md` / `AGENTS.md` inherits this rule. When the next constitution regeneration runs, this rule MUST appear in CONSTITUTION.md as CONST-035 alongside the other governance rules.

    Why: drainage cycles in this repo have repeatedly discovered features that were "tested" but didn't work — chromadb tests probing the wrong port, redis "reachability" tolerating any timeout, MCP servers reported "running" because the container was Up even when the application inside was crash-looping. Worked example: BUGFIXES.md Issue #50 — every previous compose-resource test passed because they only validated structure (do all services have a `deploy:` block?) without validating that compose v2 would accept the project. CONST-035 closes that class of bug at the test layer.

    Verification of CONST-035 itself: run any test or Challenge in this repo, modify the underlying feature to be deliberately broken (e.g. `kill helixagent-postgres`), and verify the test FAILS. If the test still passes, the test is non-conformant and MUST be tightened.

    **End-user usability mandate (added 2026-04-29).** A test or Challenge that passes is a CLAIM that the tested behavior **works for the end user of the product**. We have repeatedly hit the failure mode where every test executed green AND every Challenge reported PASS, yet most product features did not actually work — buggy challenge wrappers masked failed assertions, scripts checked file existence without executing the file, "reachability" tests tolerated timeouts, contracts were honest in advertising but broken in dispatch. **This MUST NOT recur.** The pass result of every test and Challenge MUST guarantee:

       a. **Quality** — the feature behaves correctly under the inputs an end user will send, including malformed input, edge cases, and concurrency that real workloads produce.
    ...
    A passing test that doesn't certify all three of (a), (b), (c) is a bluff and MUST be tightened or marked `t.Skip("...SKIP-OK: #<ticket>")` so absence of coverage is loud rather than silent.

    **Bluff taxonomy** (each pattern observed in this repo and now forbidden):
    - **Wrapper bluff** — assertions PASS but the wrapper script's exit-code logic is buggy, marking the run FAILED. Or the inverse: assertions FAIL but the wrapper swallows them. Both directions: examined first across the 178 grep-c-double-emit scripts in this repo (commits b0deae29 and 05a50d93). Mitigation: every aggregating wrapper MUST end with `exit_code = (failed_count > 0)` and use a robust counter (`! grep -qs "|FAILED|" "$LOG"` style) — never inline arithmetic on the output of a command that prints AND exits non-zero.
    - **Contract bluff** — the system advertises a capability (`/v1/models` lists name X) but rejects it in dispatch. Mitigation: every advertised capability MUST be exercised by a Challenge that actually invokes it — see `chat_error_handling_challenge.sh` for the canonical pattern.
    - **Structural bluff** — `check_file_exists "/path/foo_test.go"` passes if the file is present, but doesn't run the test in it or assert anything about its content. Mitigation: file-existence checks MUST be paired with at least one functional assertion (`grep -q "TestXxx"` for content, or actually `go test` the file).
    - **Comment bluff** — code comment promises a behavior the code doesn't actually have (e.g. "fail-open during cold-start" but the code is fail-closed). Caught in commit 33e7c4b7 introducing the bug, fixed in the next commit. Mitigation: documentation written before / about code MUST be re-verified against the code on every change touching the documented function.
    - **Skip bluff** — `t.Skip("not running yet")` without a SKIP-OK marker silently passes when it shouldn't. Mitigation: per CLAUDE.md DoD rule 4, every skip needs `SKIP-OK: #<ticket>` and `make ci-validate-all` fails on bare skips.

    The list above is illustrative, not exhaustive. Every Challenge or test added going forward MUST pass an honest self-review against this taxonomy before being committed.

26. **Capability-aware Placement** — Placement is not just structural ("one host per service") but semantic ("the right host for each service"). Every container's compose entry MAY declare hard constraints (`helixagent.placement.require.{gpu,runtime,arch}`) and soft preferences (`helixagent.placement.prefer.{storage,memory,network}`). At boot, `bin/helixagent` runs a `CapabilityProber` against each registered remote host (one SSH round-trip per host: `uname -m`, `docker/podman version`, `nvidia-smi`/`lspci`, `/proc/meminfo`, `/sys/block/*/queue/rotational`) and scores every (group, host) pair via `internal/placement.ScoreHost`. Hard constraints gate eligibility; soft preferences are weighted (storage=10, memory=8, network=5); load count subtracts 3× per existing placement; alphabetical tie-break for determinism. Operator overrides via `CONTAINERS_REMOTE_HOST_N_LABELS=storage=fast,memory=high,network=high` win over probed values. Auto-derived: a `deploy.resources.reservations.devices.driver: nvidia` entry implicitly sets `require.gpu=nvidia`; `limits.memory ≥ 8 GiB` implicitly sets `prefer.memory=high`. Persisted plan in `<project>/.placement-plan-<basename>.json` records the reason for each placement. Enforced by `challenges/scripts/capability_aware_placement_challenge.sh` (CONST-032 guard) + `internal/placement/{capability,prober}_test.go` (15 unit tests + 2 benchmarks under `go test ./...`). Architecture, scoring formula, and labels reference: `docs/development/container-placement.md`. Worked example: `docs/issues/fixed/BUGFIXES.md` Issue #53.

25. **Partitioned Distribution / No Duplicate Containers** — When `CONTAINERS_REMOTE_ENABLED=true`, every containerized service runs on **EXACTLY ONE** host across the registered `CONTAINERS_REMOTE_HOST_N_*` set. Replication across hosts is FORBIDDEN — multiple postgres instances each accepting writes diverge silently and corrupt data. Placement is computed at boot by `internal/placement.PlanCompose` using the existing `Containers/pkg/scheduler` (default `StrategyResourceAware`); no external scripts touch container state per CONST-031. Services with `depends_on` form a co-location group and are placed atomically (cognee + postgres + redis + chromadb always co-located). Per-compose plans are persisted to `<project>/.placement-plan-<basename>.json` for operator audit. Gateway routing uses `SVC_<SERVICE>_HOST` env vars set after each successful deploy. Enforced by `challenges/scripts/partitioned_distribution_challenge.sh` (CONST-032 guard) and `internal/placement/*_test.go` (no-duplicates / co-location invariants under `go test`). Architecture + rationale: `docs/development/container-placement.md`. Worked example: `docs/issues/fixed/BUGFIXES.md` Issue #52 (replicated postgres on thinker + amber → divergent state). Adding a service: declare its `depends_on:` correctly so co-location is automatic; no manual placement config needed.
