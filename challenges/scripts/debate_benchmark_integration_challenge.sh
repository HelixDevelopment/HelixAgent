...
if [ -f "$PROJECT_ROOT/DebateOrchestrator/evaluation/benchmark_bridge.go" ]; then
...
if (cd "$PROJECT_ROOT/DebateOrchestrator" && go build ./evaluation/... 2>&1); then
...
if grep -q "type BenchmarkBridge struct" "$PROJECT_ROOT/DebateOrchestrator/evaluation/benchmark_bridge.go" 2>/dev/null; then
...
if grep -q "type EvaluationScore struct" "$PROJECT_ROOT/DebateOrchestrator/evaluation/benchmark_bridge.go" 2>/dev/null; then
...
if grep -q "type DebateBenchmarkSuite struct" "$PROJECT_ROOT/DebateOrchestrator/evaluation/benchmark_bridge.go" 2>/dev/null; then
...
if grep -q "func NewBenchmarkBridge" "$PROJECT_ROOT/DebateOrchestrator/evaluation/benchmark_bridge.go" 2>/dev/null; then
...
if grep -q "func (b \*BenchmarkBridge) EvaluateDebateResult" "$PROJECT_ROOT/DebateOrchestrator/evaluation/benchmark_bridge.go" 2>/dev/null; then
...
if grep -q "type BenchmarkProblem struct" "$PROJECT_ROOT/DebateOrchestrator/evaluation/benchmark_bridge.go" 2>/dev/null; then
...
if [ -f "$PROJECT_ROOT/DebateOrchestrator/evaluation/benchmark_bridge_test.go" ]; then
...
if (cd "$PROJECT_ROOT/DebateOrchestrator" && nice -n 19 ionice -c 3 go test -short -count=1 -p 1 -timeout 120s ./evaluation/ 2>&1 | tail -5 | grep -q "^ok\|PASS"); then
