...
func TestNewProviderWithCustomURL(t *testing.T) {
	...
	provider := NewKimiProvider("test-key", customURL, "custom-model")
...
func TestNewProviderWithRetry(t *testing.T) {
	...
	provider := NewKimiProviderWithRetry("test-key", "", "", retryConfig)
...
func TestComplete(t *testing.T) {
	...
	assert.Equal(t, "kimi", resp.ProviderID)
}
...
func TestValidateConfig(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			provider := NewKimiProvider(tt.apiKey, "", "")
