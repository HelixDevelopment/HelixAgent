...
func TestHealthCheck_HonorsInjectedBaseURL(t *testing.T) {
	...
	p := NewProvider("gsk_test-key", server.URL+"/openai/v1/chat/completions", "")
	err := p.HealthCheck()
	require.NoError(t, err)
	assert.Equal(t, int32(1), atomic.LoadInt32(&hit), "HealthCheck must hit the injected server, not the production constant")
	assert.Equal(t, "/openai/v1/models", gotPath, "models endpoint must be derived from the injected base URL")
}
