...
func TestNewProviderWithCustomURL(t *testing.T) {
	...
	provider := NewVulavulaProvider("test-key", customURL, "custom-model")
...
func TestNewProviderWithRetry(t *testing.T) {
	...
	provider := NewVulavulaProviderWithRetry("test-key", "", "", retryConfig)
...
func TestComplete(t *testing.T) {
	...
	provider := NewVulavulaProvider("test-api-key", server.URL, "")
	...
	assert.Equal(t, "vulavula", resp.ProviderID)
}
...
func TestCompleteWithError(t *testing.T) {
	...
	provider := NewVulavulaProvider("invalid-key", server.URL, "")
...
func TestCompleteStream(t *testing.T) {
	...
	provider := NewVulavulaProvider("test-key", server.URL, "")
...
func TestGetCapabilities(t *testing.T) {
	t.Parallel()
	provider := NewVulavulaProvider("test-key", "", "")
...
func TestValidateConfig(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			provider := NewVulavulaProvider(tt.apiKey, "", "")
...
func BenchmarkVulavulaProvider_ConvertRequest(b *testing.B) {
	provider := NewVulavulaProvider("test-key", "", "")
	req := &models.LLMRequest{
		ID: "bench-request",
...
func BenchmarkVulavulaProvider_GetCapabilities(b *testing.B) {
	provider := NewVulavulaProvider("test-key", "", "")
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
