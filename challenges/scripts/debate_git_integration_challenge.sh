...
if [ -f "$PROJECT_ROOT/DebateOrchestrator/tools/git_tool.go" ]; then
...
if (cd "$PROJECT_ROOT/DebateOrchestrator" && go build ./tools/... 2>&1); then
...
if grep -q "type GitTool struct" "$PROJECT_ROOT/DebateOrchestrator/tools/git_tool.go" 2>/dev/null; then
...
if grep -q "type GitToolConfig struct" "$PROJECT_ROOT/DebateOrchestrator/tools/git_tool.go" 2>/dev/null; then
...
if grep -q "func NewGitTool" "$PROJECT_ROOT/DebateOrchestrator/tools/git_tool.go" 2>/dev/null; then
...
if grep -q "func (g \*GitTool) CreateWorktree" "$PROJECT_ROOT/DebateOrchestrator/tools/git_tool.go" 2>/dev/null; then
...
if grep -q "func (g \*GitTool) CommitSnapshot" "$PROJECT_ROOT/DebateOrchestrator/tools/git_tool.go" 2>/dev/null; then
...
if grep -q "func (g \*GitTool) Cleanup" "$PROJECT_ROOT/DebateOrchestrator/tools/git_tool.go" 2>/dev/null; then
...
if grep -q "type WorktreeInfo struct" "$PROJECT_ROOT/DebateOrchestrator/tools/git_tool.go" 2>/dev/null; then
...
if [ -f "$PROJECT_ROOT/DebateOrchestrator/tools/git_tool_test.go" ]; then
...
if command -v git &>/dev/null; then
    if (cd "$PROJECT_ROOT/DebateOrchestrator" && nice -n 19 ionice -c 3 go test -short -count=1 -p 1 -timeout 120s ./tools/ -run "TestGit|TestNewGit|TestWorktree|TestSnapshot" 2>&1 | tail -5 | grep -q "^ok\|PASS"); then
