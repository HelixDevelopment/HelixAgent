...
func TestNewProvider(t *testing.T) {
	t.Parallel()
	provider := NewNiaProvider("test-api-key", "", "")
	assert.NotNil(t, provider)
	assert.Equal(t, "test-api-key", provider.apiKey)
	assert.Equal(t, "https://api.nia.ai/v1/chat/completions", provider.baseURL)
	assert.Equal(t, "nia-1.5", provider.model)
}

func TestNewProviderWithCustomURL(t *testing.T) {
	t.Parallel()
	customURL := "https://custom.api.com/v1/chat/completions"
	provider := NewNiaProvider("test-key", customURL, "custom-model")
...
func TestNewProviderWithRetry(t *testing.T) {
	...
	provider := NewNiaProviderWithRetry("test-key", "", "", retryConfig)
...
func TestComplete(t *testing.T) {
	...
	provider := NewNiaProvider("test-api-key", server.URL, "")
	...
	assert.Equal(t, "nia", resp.ProviderID)
}
...
func TestCompleteWithError(t *testing.T) {
	...
	provider := NewNiaProvider("invalid-key", server.URL, "")
...
func TestCompleteStream(t *testing.T) {
	...
	provider := NewNiaProvider("test-key", server.URL, "")
...
func TestGetCapabilities(t *testing.T) {
	t.Parallel()
	provider := NewNiaProvider("test-key", "", "")
	...
	assert.Equal(t, 32768, caps.Limits.MaxInputLength)
}
...
func TestValidateConfig(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			provider := NewNiaProvider(tt.apiKey, "", "")
...
func BenchmarkNiaProvider_ConvertRequest(b *testing.B) {
	provider := NewNiaProvider("test-key", "", "")
	req := &models.LLMRequest{
		ID: "bench-request",
...
func BenchmarkNiaProvider_GetCapabilities(b *testing.B) {
	provider := NewNiaProvider("test-key", "", "")
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
