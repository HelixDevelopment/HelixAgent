...
if [ -f "$PROJECT_ROOT/DebateOrchestrator/protocol/dehallucination.go" ]; then
...
if grep -q "type DehallucationPhase struct" "$PROJECT_ROOT/DebateOrchestrator/protocol/dehallucination.go" 2>/dev/null; then
...
if grep -q "type DehallucationConfig struct" "$PROJECT_ROOT/DebateOrchestrator/protocol/dehallucination.go" 2>/dev/null; then
...
if grep -q "type ClarificationRequest struct" "$PROJECT_ROOT/DebateOrchestrator/protocol/dehallucination.go" 2>/dev/null; then
...
if grep -q "func NewDehallucationPhase" "$PROJECT_ROOT/DebateOrchestrator/protocol/dehallucination.go" 2>/dev/null; then
...
if grep -q 'PhaseDehallucination.*DebatePhase.*=.*"dehallucination"' "$PROJECT_ROOT/DebateOrchestrator/topology/topology.go" 2>/dev/null; then
...
PHASE_COUNT=$(grep -c 'Phase[A-Z].*DebatePhase.*=' "$PROJECT_ROOT/DebateOrchestrator/topology/topology.go" 2>/dev/null || echo "0")
...
if [ -f "$PROJECT_ROOT/DebateOrchestrator/protocol/dehallucination_test.go" ]; then
...
if (cd "$PROJECT_ROOT/DebateOrchestrator" && nice -n 19 ionice -c 3 go test -short -count=1 -p 1 -timeout 120s ./protocol/ -run "TestDehallu|TestClarif|TestNewDehallu" 2>&1 | tail -5 | grep -q "^ok\|PASS"); then
