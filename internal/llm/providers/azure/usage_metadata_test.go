...
// TestProvider_Complete_RecordsRealTokenSplit guards the 2026-09-03
// change that stopped this provider parsing the upstream per-direction
// token counts and then keeping only the total.
//
// models.LLMResponse.TokenSplit reads these Metadata keys to build the
// OpenAI-compatible usage envelope, so discarding them made every Azure
// response publish prompt_tokens=0 / completion_tokens=0 for values
// Azure had measured and returned.
func TestProvider_Complete_RecordsRealTokenSplit(t *testing.T) {
	// Asymmetric split with an ODD total — a 50/50 halving of 41 gives
	// 20/20, so it cannot satisfy these assertions by accident.
	const body = `{
	...
	p := NewProvider(srv.URL, "test-deployment", "test-key")
