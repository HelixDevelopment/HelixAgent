...
func TestNewProviderWithCustomURL(t *testing.T) {
	...
	provider := NewNvidiaProvider("test-key", customURL, "custom-model")
...
func TestNewProviderWithRetry(t *testing.T) {
	...
	provider := NewNvidiaProviderWithRetry("test-key", "", "", retryConfig)
...
func TestComplete(t *testing.T) {
	...
	provider := NewNvidiaProvider("test-api-key", server.URL, "")
	...
	assert.Equal(t, "nvidia", resp.ProviderID)
}
...
func TestCompleteWithError(t *testing.T) {
	...
	provider := NewNvidiaProvider("invalid-key", server.URL, "")
...
func TestCompleteStream(t *testing.T) {
	...
	provider := NewNvidiaProvider("test-key", server.URL, "")
...
func TestGetCapabilities(t *testing.T) {
	...
	assert.Equal(t, 16384, caps.Limits.MaxTokens)
...
func TestValidateConfig(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
				t.Parallel()
			provider := NewNvidiaProvider(tt.apiKey, "", "")
...
func TestHealthCheckWithError(t *testing.T) {
	...
	// For now, just verify health check returns an error with invalid key
	provider := NewNvidiaProvider("invalid-key-for-testing-only", "", "")
	err := provider.HealthCheck()
	// This may or may not error depending on network
