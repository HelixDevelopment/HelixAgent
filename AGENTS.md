...
CONST-036 closes that loophole at both the source-code layer (no
command you generate may directly terminate a session) and the
operational layer (you SHOULD NOT spawn workloads that will
plausibly force a manual logout).
See `docs/issues/fixed/SESSION_LOSS_2026-04-28.md`.
...
2. Long-lived background subagents go in `system.slice`, not
   `user.slice` — rootless podman containers die with the user
   manager, so anything that must survive a desktop crash needs
   a system unit (or rootful podman).
3. Document AI-agent concurrency limits in CLAUDE.md per submodule.
...
**Full background:**
`docs/issues/fixed/SESSION_LOSS_2026-04-28.md` and
`CONSTITUTION.md` (CONST-036).
