...
func TestNewProviderWithCustomURL(t *testing.T) {
	...
	provider := NewCodestralProvider("test-key", customURL, "custom-model")
...
func TestNewProviderWithRetry(t *testing.T) {
	...
	provider := NewCodestralProviderWithRetry("test-key", "", "", retryConfig)
...
func TestComplete(t *testing.T) {
	...
	provider := NewCodestralProvider("test-api-key", server.URL, "")
	...
	assert.Equal(t, "codestral", resp.ProviderID)
}
...
func TestCompleteStream(t *testing.T) {
	...
	provider := NewCodestralProvider("test-key", server.URL, "")
...
func TestGetCapabilities(t *testing.T) {
	t.Parallel()
	provider := NewCodestralProvider("test-key", "", "")
...
func BenchmarkCodestralProvider_ConvertRequest(b *testing.B) {
	provider := NewCodestralProvider("test-key", "", "")
	req := &models.LLMRequest{
		ID: "bench-request",
...
func BenchmarkCodestralProvider_GetCapabilities(b *testing.B) {
	provider := NewCodestralProvider("test-key", "", "")
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
