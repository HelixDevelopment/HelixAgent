...
func TestNewProviderWithCustomURL(t *testing.T) {
	...
	provider := NewKiloProvider("test-key", customURL, "custom-model")
...
func TestNewProviderWithRetry(t *testing.T) {
	...
	provider := NewKiloProviderWithRetry("test-key", "", "", retryConfig)
...
func TestComplete(t *testing.T) {
	...
	provider := NewKiloProvider("test-api-key", server.URL, "")
	...
	assert.Equal(t, "kilo", resp.ProviderID)
}
...
func TestCompleteWithError(t *testing.T) {
	...
	provider := NewKiloProvider("invalid-key", server.URL, "")
...
func TestCompleteStream(t *testing.T) {
	...
	provider := NewKiloProvider("test-key", server.URL, "")
...
func TestGetCapabilities(t *testing.T) {
	t.Parallel()
	provider := NewKiloProvider("test-key", "", "")
...
func TestValidateConfig(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			provider := NewKiloProvider(tt.apiKey, "", "")
...
func BenchmarkKiloProvider_ConvertRequest(b *testing.B) {
	provider := NewKiloProvider("test-key", "", "")
	req := &models.LLMRequest{
		ID: "bench-request",
...
func BenchmarkKiloProvider_GetCapabilities(b *testing.B) {
	provider := NewKiloProvider("test-key", "", "")
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
