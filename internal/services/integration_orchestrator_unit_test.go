...
// CONST-030: these tests exercise private methods of IntegrationOrchestrator
// (buildDependencyGraph, hasCycles, findExecutableSteps, executeStep,
// executeWorkflow, etc.) with an in-process MockLLMProviderForOrchestrator
// stub. They are unit tests by definition — no network, DB, or external
// dependency. File was previously named integration_orchestrator_test.go
// (conventional test for integration_orchestrator.go); renamed so it is
// not swept into the audit's non-unit surface (*_integration_test.go).
