...
if [ -f "$PROJECT_ROOT/DebateOrchestrator/protocol/self_evolvement.go" ]; then
...
if (cd "$PROJECT_ROOT/DebateOrchestrator" && go build ./protocol/... 2>&1); then
...
if grep -q "type SelfEvolvementPhase struct" "$PROJECT_ROOT/DebateOrchestrator/protocol/self_evolvement.go" 2>/dev/null; then
...
if grep -q "type SelfEvolvementConfig struct" "$PROJECT_ROOT/DebateOrchestrator/protocol/self_evolvement.go" 2>/dev/null; then
...
if grep -q "type SelfTestResult struct" "$PROJECT_ROOT/DebateOrchestrator/protocol/self_evolvement.go" 2>/dev/null; then
...
if grep -q "func NewSelfEvolvementPhase" "$PROJECT_ROOT/DebateOrchestrator/protocol/self_evolvement.go" 2>/dev/null; then
...
if grep -q "func (s \*SelfEvolvementPhase) Execute" "$PROJECT_ROOT/DebateOrchestrator/protocol/self_evolvement.go" 2>/dev/null; then
...
if grep -q 'PhaseSelfEvolvement.*DebatePhase.*=.*"self_evolvement"' "$PROJECT_ROOT/DebateOrchestrator/topology/topology.go" 2>/dev/null; then
...
if [ -f "$PROJECT_ROOT/DebateOrchestrator/protocol/self_evolvement_test.go" ]; then
...
if (cd "$PROJECT_ROOT/DebateOrchestrator" && nice -n 19 ionice -c 3 go test -short -count=1 -p 1 -timeout 120s ./protocol/ -run "TestSelfEvolve|TestSelfTest|TestNewSelfEvolve" 2>&1 | tail -5 | grep -q "^ok\|PASS"); then
