...
func TestTreeOfThoughts_Solve_BFS(t *testing.T) {
	...
	// Anti-bluff: root alone gives NodesExplored=1, so > MaxBranches proves
	// the algorithm at least expanded one level. But the load-bearing
	// assertion is that initial-grafted thoughts (ID prefix "init-",
	// generated by Mock.GenerateInitialThoughts) actually became root
	// children — without that loop, root would have only BFS-generated
	// children (which have parent-derived ID like "root-child-N"), never
	// "init-N". (close-out⁶⁹ hardening: original `NodesExplored > 0`
	// passed even when the entire initial-grafting loop was removed.)
	assert.Greater(t, result.Iterations, 0,
		"Iterations must be > 0: proves BFS actually dequeued nodes")
	initGraftedCount := 0
	for _, c := range tot.root.Children {
		if len(c.Thought.ID) >= 5 && c.Thought.ID[:5] == "init-" {
			initGraftedCount++
		}
	}
	assert.Equal(t, config.MaxBranches, initGraftedCount,
		"root must have exactly MaxBranches initial-grafted children (ID prefix 'init-'); fewer = initial-thoughts grafting was skipped (close-out⁶⁹ bluff pattern)")
}
...
func TestTreeOfThoughts_Solve_DFS(t *testing.T) {
	...
	// Anti-bluff (close-out⁶⁹ companion): same initial-grafting test as BFS.
	assert.Greater(t, result.Iterations, 0,
		"Iterations must be > 0: proves DFS actually descended")
	initGraftedCount := 0
	for _, c := range tot.root.Children {
		if len(c.Thought.ID) >= 5 && c.Thought.ID[:5] == "init-" {
			initGraftedCount++
	...
	assert.Equal(t, config.MaxBranches, initGraftedCount,
		"root must have exactly MaxBranches initial-grafted children (ID prefix 'init-')")
}
...
func TestTreeOfThoughts_Solve_Beam(t *testing.T) {
	...
	// Anti-bluff (close-out⁶⁹ companion): same initial-grafting test as BFS.
	assert.Greater(t, result.Iterations, 0,
		"Iterations must be > 0: proves beam search actually iterated")
	initGraftedCount := 0
	for _, c := range tot.root.Children {
		if len(c.Thought.ID) >= 5 && c.Thought.ID[:5] == "init-" {
			initGraftedCount++
