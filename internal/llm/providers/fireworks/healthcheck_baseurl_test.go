...
func TestHealthCheck_HonorsInjectedBaseURL(t *testing.T) {
	...
	p := NewProvider("test-key", server.URL+"/inference/v1/chat/completions", "")
	err := p.HealthCheck()
	require.NoError(t, err)
	assert.Equal(t, int32(1), atomic.LoadInt32(&hit), "HealthCheck must hit the injected server, not the production constant")
	assert.Equal(t, "/inference/v1/models", gotPath, "models endpoint must be derived from the injected base URL")
}
