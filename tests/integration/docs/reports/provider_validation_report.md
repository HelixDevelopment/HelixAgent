...
| 3 | gemini | `GEMINI_API_KEY` | AIza...w8Lg (39 chars) | FAIL bad_request | 183ms | Provider gemini (GEMINI_API_KEY) bad request (HTTP 400). Error: API Key not f... |
| 4 | deepseek | `DEEPSEEK_API_KEY` | sk-c...3935 (35 chars) | OK healthy | 408ms |  |
| 5 | mistral | `MISTRAL_API_KEY` | XBne...kSAH (32 chars) | OK healthy | 613ms |  |
| 6 | codestral | `CODESTRAL_API_KEY` | 3AFH...yiII (32 chars) | OK healthy | 253ms |  |
...
| 9 | zai | `ZAI_API_KEY` | 8dd4...Bps0 (49 chars) | RATE rate_limited | 577ms | Provider zai (ZAI_API_KEY) rate limited. The API key is valid but quota is ex... |
| 10 | github-models | `GITHUB_MODELS_API_KEY` | gith...hkbp (93 chars) | AUTH auth_failed | 443ms | Provider github-models (GITHUB_MODELS_API_KEY) authentication failed (HTTP 40... |
| 11 | cohere | `COHERE_API_KEY` | banh...mI9o (40 chars) | AUTH auth_failed | 333ms | Provider cohere (COHERE_API_KEY) authentication failed (HTTP 403). The API ke... |
...
| 14 | groq | `GROQ_API_KEY` | gsk_...7xhr (56 chars) | AUTH auth_failed | 109ms | Provider groq (GROQ_API_KEY) authentication failed (HTTP 403). The API key is... |
| 15 | cerebras | `CEREBRAS_API_KEY` | csk-...htj2 (52 chars) | AUTH auth_failed | 144ms | Provider cerebras (CEREBRAS_API_KEY) authentication failed (HTTP 403). The AP... |
| 16 | sambanova | `SAMBANOVA_API_KEY` | 3eb0...1533 (36 chars) | FAIL model_not_found | 636ms | Provider sambanova (SAMBANOVA_API_KEY) model Meta-Llama-3.1-70B-Instruct not ... |
| 17 | fireworks | `FIREWORKS_API_KEY` | fw_J...ezWb (25 chars) | FAIL model_not_found | 160ms | Provider fireworks (FIREWORKS_API_KEY) model accounts/fireworks/models/llama-... |
| 18 | together | `TOGETHER_API_KEY` | N/A | KEY no_key |  | Environment variable TOGETHER_API_KEY is not set. Set it with: export TOGETHE... |
| 19 | hyperbolic | `HYPERBOLIC_API_KEY` | sk_l...W720 (73 chars) | AUTH auth_failed | 93ms | Provider hyperbolic (HYPERBOLIC_API_KEY) authentication failed (HTTP 403). Th... |
| 20 | replicate | `REPLICATE_API_KEY` | r8_4...uxlG (40 chars) | AUTH auth_failed | 480ms | Provider replicate (REPLICATE_API_KEY) authentication failed (HTTP 401). The ... |
| 21 | siliconflow | `SILICONFLOW_API_KEY` | sk-r...kdno (51 chars) | AUTH auth_failed | 643ms | Provider siliconflow (SILICONFLOW_API_KEY) authentication failed (HTTP 401). ... |
| 22 | cloudflare | `CLOUDFLARE_API_KEY` | h30D...RPhR (40 chars) | FAIL bad_request | 324ms | Provider cloudflare (CLOUDFLARE_API_KEY) bad request (HTTP 400). Error: {"err... |
| 23 | nvidia | `NVIDIA_API_KEY` | nvap...-Tlx (70 chars) | OK healthy | 17312ms |  |
| 24 | kimi | `KIMI_API_KEY` | sk-k...JvbE (72 chars) | AUTH auth_failed | 994ms | Provider kimi (KIMI_API_KEY) authentication failed (HTTP 401). The API key is... |
| 25 | huggingface | `HUGGINGFACE_API_KEY` | hf_e...CpNo (37 chars) | FAIL model_not_found | 260ms | Provider huggingface (HUGGINGFACE_API_KEY) model meta-llama/Llama-3.2-3B-Inst... |
| 26 | novita | `NOVITA_API_KEY` | sk_l...eheo (46 chars) | FAIL model_not_found | 294ms | Provider novita (NOVITA_API_KEY) model meta-llama/llama-3.1-70b-instruct not ... |
| 27 | upstage | `UPSTAGE_API_KEY` | up_S...xhCu (32 chars) | AUTH auth_failed | 1167ms | Provider upstage (UPSTAGE_API_KEY) authentication failed (HTTP 401). The API ... |
| 28 | chutes | `CHUTES_API_KEY` | cpk_...OyQR (102 chars) | FAIL model_not_found | 856ms | Provider chutes (CHUTES_API_KEY) model chutesai/Chutes-Mistral-Nemo-2407 not ... |
| 29 | openrouter | `OPENROUTER_API_KEY` | sk-o...4462 (73 chars) | FAIL model_not_found | 319ms | Provider openrouter (OPENROUTER_API_KEY) model x-ai/grok-4 not found (HTTP 40... |
| 30 | venice | `VENICE_API_KEY` | VENI...GKbU (59 chars) | FAIL model_not_found | 265ms | Provider venice (VENICE_API_KEY) model llama-3.1-70b-instruct not found (HTTP... |
| 31 | sarvam | `SARVAM_API_KEY` | sk_1...bjPY (36 chars) | OK healthy | 1131ms |  |
| 32 | kilo | `KILO_API_KEY` | eyJh...MozU (268 chars) | AUTH auth_failed | 108ms | Provider kilo (KILO_API_KEY) authentication failed (HTTP 403). The API key is... |
| 33 | publicai | `PUBLICAI_API_KEY` | zpka...1d43 (46 chars) | AUTH auth_failed | 166ms | Provider publicai (PUBLICAI_API_KEY) authentication failed (HTTP 401). The AP... |
| 34 | modal | `MODAL_API_KEY` | as-c...3x8Z (25 chars) | OK healthy | 506ms |  |
| 35 | nia | `NIA_API_KEY` | nk_h...PEsw (35 chars) | NET dns_failure | 37ms | Provider nia (NIA_API_KEY) DNS resolution failed for https://api.nia.ai/v1/ch... |
| 36 | nvidia | `NVIDIA_API_KEY` | nvap...-Tlx (70 chars) | OK healthy | 673ms |  |

---

## Healthy Providers

- **deepseek** (`DEEPSEEK_API_KEY`) — OK in 408ms
- **mistral** (`MISTRAL_API_KEY`) — OK in 613ms
- **codestral** (`CODESTRAL_API_KEY`) — OK in 253ms
- **nvidia** (`NVIDIA_API_KEY`) — OK in 17312ms
- **sarvam** (`SARVAM_API_KEY`) — OK in 1131ms
- **modal** (`MODAL_API_KEY`) — OK in 506ms
- **nvidia** (`NVIDIA_API_KEY`) — OK in 673ms
