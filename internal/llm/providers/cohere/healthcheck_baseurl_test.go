...
// models endpoint from the injected base URL (the chat URL) instead of the
// hardcoded production constant, so a proxy / httptest base URL is honored.
// Cohere's chat endpoint is "/v2/chat" while models live at "/v1/models".
func TestHealthCheck_HonorsInjectedBaseURL(t *testing.T) {
	...
	p := NewProvider("test-api-key", server.URL+"/v2/chat", "")
	err := p.HealthCheck()
