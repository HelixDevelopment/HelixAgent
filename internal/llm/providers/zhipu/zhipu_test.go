...
func TestNewProvider(t *testing.T) {
	...
	assert.Equal(t, "https://open.bigmodel.cn/api/paas/v4/chat/completions", provider.baseURL)
	assert.Equal(t, "glm-4-flash", provider.model)
}
...
func TestNewProviderWithRetry(t *testing.T) {
	...
	provider := NewZhipuProviderWithRetry("test-key", "", "", retryConfig)
...
func TestComplete(t *testing.T) {
	...
	provider := NewZhipuProvider("test-api-key", server.URL, "")
	...
	assert.Equal(t, "zhipu", resp.ProviderID)
}
...
func TestCompleteWithError(t *testing.T) {
	...
	provider := NewZhipuProvider("invalid-key", server.URL, "")
...
func TestCompleteStream(t *testing.T) {
	...
	provider := NewZhipuProvider("test-key", server.URL, "")
...
func BenchmarkZhipuProvider_ConvertRequest(b *testing.B) {
	provider := NewZhipuProvider("test-key", "", "")
	req := &models.LLMRequest{
		ID: "bench-request",
...
func BenchmarkZhipuProvider_GetCapabilities(b *testing.B) {
	provider := NewZhipuProvider("test-key", "", "")
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
