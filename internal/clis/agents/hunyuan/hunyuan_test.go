...
func TestHunyuan_Execute(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// Reconciled (§11.4.120): chat USED to return the fabricated
			// "Hunyuan: Hello" template with wantErr:false — that codified
			// BLUFF-001. Hunyuan has NO headless CLI; with no real backend wired
			// chat now returns an HONEST error.
			name:    "chat command (honest error — no real backend)",
			command: "chat",
			params:  map[string]interface{}{"message": "Hello"},
			wantErr: true,
			errMsg:  "no headless CLI exists",
		},
		{
		...
		{
			// Reconciled (§11.4.120): generate USED to return the fabricated
			// "// Generated by Hunyuan" template with wantErr:false — BLUFF-001.
			// Honest error now.
			name:    "generate command (honest error — no real backend)",
			command: "generate",
